...
export const crmRepository={
  customers:async(values:Record<string,string|number>={})=>{const page=await apiRequest<PageResult<Customer>>(`/api/crm/customers?${query(values)}`);return{...page,items:page.items.map(item=>({...item,tags:parseJsonArray(item.tags)}))}},
  ...
  leads:async(values:Record<string,string|number>={})=>{const page=await apiRequest<PageResult<Lead>>(`/api/crm/leads?${query(values)}`);return{...page,items:page.items.map(item=>({...item,products:parseJsonArray(item.products)}))}},
  ...
  opportunities:async(values:Record<string,string|number>={})=>{const page=await apiRequest<PageResult<Opportunity>>(`/api/crm/opportunities?${query(values)}`);return{...page,items:page.items.map(item=>({...item,collaborators:parseJsonArray(item.collaborators)}))}},
  ...
  followUps:async(values:Record<string,string|number>={})=>{const page=await apiRequest<PageResult<FollowUpRecord>>(`/api/crm/follow-ups?${query(values)}`);return{...page,items:page.items.map(item=>({...item,method:item.method||item.followType,date:item.date||item.followTime,attachments:parseJsonArray(item.attachments)}))}},
  ...
  contracts:(values:Record<string,string|number>={})=>apiRequest<PageResult<Contract>>(`/api/crm/contracts?${query(values)}`),
  ...
  journey:async(values:Record<string,string|number>={})=>apiRequest<PageResult<CrmJourneyEvent>>(`/api/crm/journey?${query(values)}`),
