...
const DevTasksView = lazyNamed(() => import('./components/product/DevTasksView'), 'DevTasksView');
const BugManagementView = lazyNamed(() => import('./components/product/BugManagementView'), 'BugManagementView');
...
const MainContent: React.FC = () => {
  const { activeTabId, openTabs, crmLoading, crmError, retryCrm, addToast } = useApp();
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[var(--border-main)] pb-4">
          <div>
            ...
            <h1 className="text-lg lg:text-xl font-serif font-bold text-[var(--text-primary)] mt-1 tracking-wide">
              {currentTab?.title || '业务模块'}
            </h1>
          </div>
        </div>
        ...
        <Suspense fallback={<div className="dark-panel flex min-h-48 items-center justify-center rounded-lg text-sm text-[var(--text-muted)]">正在加载页面…</div>}>
