...
import { X, AlertCircle, CheckCircle2, Info, AlertTriangle, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
export const Modal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-main)] bg-[var(--bg-nav)]">
          <h3 className="text-base font-semibold text-white font-serif tracking-wide">{title}</h3>
          <button
            id="btn-modal-close"
            onClick={onClose}
...
export const Drawer: React.FC<{
  ...
  footer?: React.ReactNode;
  width?: string;
}> = ({ isOpen, onClose, title, subtitle, children, footer, width = 'max-w-2xl' }) => {
  if (!isOpen) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 pl-10 max-w-full flex">
        ...
        >
          <div className="px-6 py-4 border-b border-[var(--border-main)] flex items-center justify-between bg-[var(--bg-nav)]">
            <div>
              <h2 className="text-base font-semibold text-white font-serif tracking-wide">{title}</h2>
      ...
      {toasts.map((toast) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-sm font-medium text-white leading-tight">{toast.title}</h4>
            {toast.message && (
