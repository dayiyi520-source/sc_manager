...
import { ArrowLeft, ClipboardCheck, Plus, Ticket } from 'lucide-react';
...
export const RequirementWorkItemsView: React.FC<{ type: 'presales' | 'delivery' }> = ({ type }) => {
  const { requirementTasks, addToast } = useApp();
  const workType: RequirementTaskType = type === 'presales' ? '售前支持' : '项目交付';
  ...
  useEffect(() => {
    ...
    requirementRepository.syncStatus({ taskType: workType, syncStatus: syncFilter, page: 1, pageSize: 100 }).then((result) => {
      ...
      if (!active) return;
      setItems(requirementTasks.filter((item) => item.taskType === workType || (type === 'delivery' && item.taskType === '交付支持')).map((item) => ({ id: item.taskId || item.id, requirementId: item.id, requirementCode: item.code, requirementTitle: item.title, taskType: workType, title: item.title, assigneeName: item.assignedOwnerName || item.ownerName, note: item.assignedNote, status: item.status, createdAt: item.createdAt })));
    }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [workType, requirementTasks, syncFilter]);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try { await requirementRepository.createWorkItem(sourceId, { title: title.trim(), taskType: workType, assigneeName: assignee.trim(), note }); setCreating(false); const result = await requirementRepository.syncStatus({ taskType: workType, syncStatus: syncFilter, page: 1, pageSize: 100 }); setItems(result.items); addToast('success', '工单已创建'); } catch (error) { addToast('error', '工单创建失败', error instanceof Error ? error.message : '请稍后重试'); }
  };
