...
<template>
  <div v-if="!editing">
    ...
    <section v-if="record.type === 'requirement'" class="work-item-acceptance"><strong>产品验收：{{record.acceptanceStatus || '待验收'}}</strong><div class="form-actions"><a-button type="default" :disabled="acceptanceBusy" @click="updateAcceptance('已驳回')">驳回验收</a-button><a-button type="primary" :loading="acceptanceBusy" @click="updateAcceptance('已通过')">通过验收</a-button></div><p v-if="feedback" class="form-feedback">{{feedback}}</p></section>
    ...
    <div class="form-actions"><a-button @click="emit('close')">关闭</a-button><a-button type="primary" @click="editing=true">编辑</a-button></div>
  </div>
