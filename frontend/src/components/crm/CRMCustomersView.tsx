...
export const CRMCustomersView: React.FC = () => {
  ...
  const customerBiddingProjects = activeCustomer
    ? biddingProjects.filter((item) => item.customerId === activeCustomer.id)
    : [];
        ...
        <section className="space-y-5 text-xs animate-in fade-in duration-150">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
            <div className="flex items-center gap-3 min-w-0">
              <button
                type="button"
                onClick={() => setSelectedCustomerIdForDetail(null)}
                className="inline-flex items-center gap-1.5 px-2.5 py-1.5 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 hover:text-blue-600 hover:border-blue-500 transition-colors"
              >
                <ChevronRight className="w-3.5 h-3.5 rotate-180" />
                返回客户列表
              </button>
              <div className="min-w-0">
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white truncate">{activeCustomer.name}</h2>
                <p className="text-xs text-slate-500 mt-0.5">客户编码：{activeCustomer.code} · {activeCustomer.level} · {activeCustomer.industry}</p>
              </div>
            </div>
            <button type="button" onClick={() => openEditModal(activeCustomer)} className="px-3 py-1.5 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 hover:text-blue-600 hover:border-blue-500 transition-colors">
              编辑档案
            </button>
          </div>
          <div className="space-y-5">
            ...
            <div className="flex border-b border-slate-200 dark:border-slate-800 gap-4 overflow-x-auto">
              <button
                ...
                className={`pb-2.5 font-semibold transition-colors border-b-2 shrink-0 ${
                  detailTab === 'profile'
            ...
            {detailTab === 'profile' && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-800/60 p-4 rounded-xl">
                  <div>
                    <span className="text-slate-400 block">客户主体类型</span>
                    <div className="font-semibold text-slate-800 dark:text-slate-200 mt-0.5">
                      {normalizeCustomerType(activeCustomer.type)}
                    </div>
                  </div>
                  <div>
                    <span className="text-slate-400 block">客户等级</span>
                    <div className="mt-0.5">
                      <StatusTag status={activeCustomer.level} />
                    </div>
                  </div>
                  <div>
                    <span className="text-slate-400 block">主联系人及职务</span>
                    <div className="font-semibold text-slate-800 dark:text-slate-200 mt-0.5">
                      {activeCustomer.contactName} ({activeCustomer.contactTitle})
                    </div>
                  </div>
                  <div>
                    <span className="text-slate-400 block">联系电话 / 邮箱</span>
                    <div className="font-semibold text-slate-800 dark:text-slate-200 mt-0.5">
                      {activeCustomer.contactPhone} {activeCustomer.contactEmail ? `· ${activeCustomer.contactEmail}` : ''}
                    </div>
                  </div>
                  <div>
                    <span className="text-slate-400 block">所属行业</span>
                    <div className="font-semibold text-slate-800 dark:text-slate-200 mt-0.5">
                      {activeCustomer.industry}
                    </div>
                  </div>
                  <div>
                    <span className="text-slate-400 block">预估年度IT预算</span>
                    <div className="font-semibold text-emerald-600 dark:text-emerald-400 mt-0.5">
                      ¥{((activeCustomer.annualBudget || 3500000) / 10000).toFixed(0)} 万元/年
                    </div>
                  </div>
                  <div>
                    <span className="text-slate-400 block">客户来源渠道</span>
                    <div className="font-semibold text-slate-800 dark:text-slate-200 mt-0.5">
                      {activeCustomer.source || '主动开发'}
                    </div>
                  </div>
                  <div>
                    <span className="text-slate-400 block">最近跟进时间</span>
                    <div className="font-semibold text-slate-800 dark:text-slate-200 mt-0.5">
                      {activeCustomer.lastFollowUp || '今日'}
                    </div>
                  </div>
                  <div className="col-span-2">
                    <span className="text-slate-400 block">注册及办公地址</span>
                    <div className="font-semibold text-slate-800 dark:text-slate-200 mt-0.5">
                      {activeCustomer.address}
                    </div>
                  </div>
                  <div className="col-span-2">
                    <span className="text-slate-400 block">业务画像标签</span>
                    <div className="flex flex-wrap gap-1.5 mt-1">
                      {activeCustomer.tags?.map((t) => (
                        <span key={t} className="px-2 py-0.5 bg-blue-100 text-blue-700 dark:bg-blue-900/60 dark:text-blue-300 rounded text-xs font-medium">
                          {t}
                        </span>
                      ))}
                    </div>
