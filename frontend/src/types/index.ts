...
export interface RequirementTask {
  ...
  priority: 'P0-紧急阻断' | 'P1-高优' | 'P2-普通' | 'P2-标准' | 'P3-低优' | string;
  ownerName: string;
  creatorName?: string;
  department?: string;
  departmentId?: string;
...
export type RequirementTaskType = '产品需求' | 'bug修复' | '售前支持' | '项目交付' | '运维部署' | '技术问题' | '其他问题';
...
export interface DevTask {
  ...
  repo?: string;
  branch?: string;
  status: '待开发' | '开发中' | '已提测' | '已合并上线';
  priority: 'P0-紧急' | 'P1-高优' | 'P2-标准';
  versionName: string;
  productLineName: string;
  estimatedHours: number;
  spentHours: number;
