...
public class UnifiedWorkItemController {
    ...
    public ApiResponse<Void> update(@PathVariable String id, @RequestBody Map<String, Object> body) {
        ...
        if (!next.isBlank() && !next.equals(String.valueOf(old.get("status_")))) { event(id, "STATUS_CHANGED", String.valueOf(old.get("status_")), next, "状态更新"); aggregateParent(id); }
    ...
    private void aggregateParent(String childId) { List<Map<String,Object>> parents=jdbc.queryForList("SELECT parent_id_ AS parentId FROM t_product_work_item WHERE id_=? AND tenant_id_=? AND parent_id_ IS NOT NULL", childId, RequestContext.tenantId()); if (parents.isEmpty()) return; String parentId=String.valueOf(parents.get(0).get("parentId")); Integer blocking=jdbc.queryForObject("SELECT COUNT(*) FROM t_product_work_item c JOIN t_product_work_item_relation rel ON rel.from_id_=c.id_ AND rel.tenant_id_=c.tenant_id_ AND rel.relation_type_='BLOCKS' WHERE rel.to_id_=? AND rel.tenant_id_=? AND c.type_='bug' AND c.status_ NOT IN ('已关闭','已完成') AND c.delete_flag_=0", Integer.class, parentId, RequestContext.tenantId()); Integer incomplete=jdbc.queryForObject("SELECT COUNT(*) FROM t_product_work_item WHERE parent_id_=? AND tenant_id_=? AND delete_flag_=0 AND status_ NOT IN ('已完成','已关闭','已验收','已发布')", Integer.class, parentId, RequestContext.tenantId()); String status=(blocking!=null&&blocking>0)?"阻塞":(incomplete==null||incomplete==0?"已完成":"部分完成"); jdbc.update("UPDATE t_product_work_item SET status_=?,update_by_=?,update_time_=NOW(),version_=version_+1 WHERE id_=? AND tenant_id_=? AND type_='requirement' AND status_<>?", status, RequestContext.userId(), parentId, RequestContext.tenantId(), status); }
