...
public class TestExecutionMapper {
    ...
    public Map<String,Object> item(String tenant,String id){return one("""
      ...
      """,tenant,id);}
    public Map<String,Object> plan(String tenant,String workItem){return one("SELECT id_ AS id,product_line_id_ AS productLineId,work_item_id_ AS workItemId,requirement_id_ AS requirementId,version_id_ AS versionId,environment_ AS environment,version_ AS revision,create_time_ AS createdAt,update_time_ AS updatedAt FROM t_product_test_plan WHERE tenant_id_=? AND work_item_id_=? AND delete_flag_=0",tenant,workItem);}
    public void insertPlan(String tenant,String id,Map<String,Object> item,String environment,String user){jdbc.update("INSERT INTO t_product_test_plan(id_,tenant_id_,product_line_id_,work_item_id_,requirement_id_,version_id_,environment_,create_by_,update_by_,create_time_,update_time_) VALUES(?,?,?,?,?,?,?,?,?,NOW(6),NOW(6))",id,tenant,item.get("productLineId"),item.get("id"),item.get("requirementId"),item.get("versionId"),environment,user,user);}
    public int updatePlan(String tenant,String id,int revision,String environment,String user){return jdbc.update("UPDATE t_product_test_plan SET environment_=?,version_=version_+1,update_by_=?,update_time_=NOW(6) WHERE tenant_id_=? AND id_=? AND version_=? AND delete_flag_=0",environment,user,tenant,id,revision);}
