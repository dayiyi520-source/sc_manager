...
public class ProductLineService {
 ...
 public List<Map<String,Object>> list(String keyword){AuthorizationService.requireRead("product"); return mapper.list(RequestContext.tenantId(),keyword==null?"":keyword.trim()).stream().map(this::enrich).toList();}
 ...
 @Transactional public void removeMember(String id,String memberId){requireWriteLine(id); Map<String,Object> member=mapper.member(RequestContext.tenantId(),id,memberId); if(member==null || mapper.removeMember(RequestContext.tenantId(),id,memberId,RequestContext.userId())==0) throw new NoSuchElementException("成员不存在"); mapper.addActivity(RequestContext.tenantId(),id,"移除成员角色",member.get("name")+" · "+member.get("role"),RequestContext.operatorName());}
