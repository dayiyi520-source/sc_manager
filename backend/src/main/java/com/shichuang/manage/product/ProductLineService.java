...
public class ProductLineService {
 ...
 public List<Map<String,Object>> members(String id){requireReadLine(id); return mapper.members(RequestContext.tenantId(),id);} @Transactional public void addMember(String id,Map<String,Object>b){requireWriteLine(id); String tenant=RequestContext.tenantId(); String name=String.valueOf(b.getOrDefault("memberName",b.get("name"))).trim(); String role=String.valueOf(b.get("role")).trim(); String userId=String.valueOf(b.getOrDefault("userId","")).trim(); if(name.isBlank()) throw new IllegalArgumentException("成员名称不能为空"); if(!Set.of("管理员","产品","研发","设计","测试","交付主管","参与人").contains(role)) throw new IllegalArgumentException("成员角色无效"); if(userId.isBlank()) userId=Objects.toString(mapper.userIdByName(tenant,name),""); if(mapper.activeMemberExists(tenant,id,userId,name)) throw new ResponseStatusException(HttpStatus.CONFLICT,"成员已在产品线中"); Map<String,Object> member=new HashMap<>(); member.put("memberName",name); member.put("role",role); member.put("userId",userId.isBlank()?null:userId); mapper.addMember(tenant,id,member,RequestContext.userId()); mapper.addActivity(tenant,id,"添加成员角色",name+" · "+role,RequestContext.operatorName());}
 ...
 private void replaceMembers(String id,List<?> members){String tenant=RequestContext.tenantId();List<Map<String,Object>> previous=mapper.members(tenant,id); mapper.removeAllMembers(tenant,id,RequestContext.userId());Set<String> added=new HashSet<>();for(Object value:members){if(value instanceof Map<?,?> raw){Map<String,Object> member=new HashMap<>(); raw.forEach((key,val)->member.put(String.valueOf(key),val));String name=memberName(member);String userId=Objects.toString(member.get("userId"),"").trim();if(userId.isBlank())userId=Objects.toString(mapper.userIdByName(tenant,name),"");String key=userId.isBlank()?"name:"+name:"user:"+userId;if(name.isBlank()||!added.add(key))continue;member.put("memberName",name);member.put("userId",userId.isBlank()?null:userId);mapper.addMember(tenant,id,member,RequestContext.userId()); mapper.addActivity(tenant,id,"添加成员角色",name+" · "+String.valueOf(member.get("role")),RequestContext.operatorName());}} for(Map<String,Object> member:previous) mapper.addActivity(tenant,id,"移除成员角色",member.get("name")+" · "+member.get("role"),RequestContext.operatorName());}
