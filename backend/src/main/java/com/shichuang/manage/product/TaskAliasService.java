...
public class TaskAliasService {
    ...
    public TaskAliasService(TaskAliasMapper mapper, WorkItemConfigurationService configurations) { this.mapper = mapper; this.configurations = configurations; }
    ...
    @Transactional public Map<String,Object> create(String type,Map<String,Object> body){
        ...
        String category = switch (type) { case "bug" -> "bug"; case "dev" -> "dev"; case "design" -> "design"; default -> null; };
        if (category != null && body.get("workItemTypeId") != null) configurations.requireTypeForLegacy(String.valueOf(body.get("productLineId")), String.valueOf(body.get("workItemTypeId")), category);
        return mapper.create(type,body);
    }
