...
public class CrmMapper {
    ...
    private static String journeySql() {
        return "SELECT CONCAT('customer:',c.id_) AS id,'客户建立' AS eventType,c.name_ AS title,CONCAT('客户状态：',COALESCE(c.status_,'')) AS description,c.status_ AS status,c.create_time_ AS occurredAt,c.tenant_id_ AS tenantId,c.id_ AS customerId,NULL AS leadId,NULL AS opportunityId,NULL AS requirementId,NULL AS workItemId,NULL AS taskType FROM t_crm_customer c WHERE c.delete_flag_=0 "
            ...
            + "UNION ALL SELECT CONCAT('presales-ticket:',p.id_) AS id,'售前工单' AS eventType,p.title_ AS title,p.note_ AS description,p.status_ AS status,p.create_time_ AS occurredAt,p.tenant_id_ AS tenantId,r.customer_id_ AS customerId,NULL AS leadId,NULL AS opportunityId,r.id_ AS requirementId,p.id_ AS workItemId,'售前支持' AS taskType FROM t_crm_presales_ticket p JOIN t_product_requirement r ON r.id_=p.requirement_id_ AND r.tenant_id_=p.tenant_id_ AND r.delete_flag_=0 WHERE p.delete_flag_=0 AND r.customer_id_ IS NOT NULL AND r.customer_id_<>'' "
            + "UNION ALL SELECT CONCAT('delivery-ticket:',d.id_) AS id,'交付工单' AS eventType,d.title_ AS title,d.note_ AS description,d.status_ AS status,d.create_time_ AS occurredAt,d.tenant_id_ AS tenantId,r.customer_id_ AS customerId,NULL AS leadId,NULL AS opportunityId,r.id_ AS requirementId,d.id_ AS workItemId,'交付支持' AS taskType FROM t_project_delivery_ticket d JOIN t_product_requirement r ON r.id_=d.requirement_id_ AND r.tenant_id_=d.tenant_id_ AND r.delete_flag_=0 WHERE d.delete_flag_=0 AND r.customer_id_ IS NOT NULL AND r.customer_id_<>''";
    }
