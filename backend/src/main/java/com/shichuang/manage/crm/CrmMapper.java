...
public class CrmMapper {
    ...
    private static String journeySql() {
        return "SELECT CONCAT('customer:',c.id_) AS id,'客户建立' AS eventType,c.name_ AS title,CONCAT('客户状态：',COALESCE(c.status_,'')) AS description,c.status_ AS status,c.create_time_ AS occurredAt,c.tenant_id_ AS tenantId,c.id_ AS customerId,NULL AS leadId,NULL AS opportunityId,NULL AS requirementId,NULL AS workItemId,NULL AS taskType FROM t_crm_customer c WHERE c.delete_flag_=0 "
            ...
            + "UNION ALL SELECT CONCAT('design-task:',d.id_) AS id,'设计任务' AS eventType,d.title_ AS title,d.description_ AS description,d.status_ AS status,d.create_time_ AS occurredAt,d.tenant_id_ AS tenantId,r.customer_id_ AS customerId,NULL AS leadId,NULL AS opportunityId,r.id_ AS requirementId,d.id_ AS workItemId,'设计任务' AS taskType FROM t_product_design_task d JOIN t_product_requirement r ON r.id_=d.requirement_id_ AND r.tenant_id_=d.tenant_id_ AND r.delete_flag_=0 WHERE d.delete_flag_=0 AND r.customer_id_ IS NOT NULL AND r.customer_id_<>'' "
            + "UNION ALL SELECT CONCAT('ops-task:',d.id_) AS id,'运维任务' AS eventType,d.title_ AS title,d.description_ AS description,d.status_ AS status,d.create_time_ AS occurredAt,d.tenant_id_ AS tenantId,r.customer_id_ AS customerId,NULL AS leadId,NULL AS opportunityId,r.id_ AS requirementId,d.id_ AS workItemId,'运维任务' AS taskType FROM t_project_ops_task d JOIN t_product_requirement r ON r.id_=d.requirement_id_ AND r.tenant_id_=d.tenant_id_ AND r.delete_flag_=0 WHERE d.delete_flag_=0 AND r.customer_id_ IS NOT NULL AND r.customer_id_<>'' "
            + "UNION ALL SELECT CONCAT('bug-task:',b.id_) AS id,'缺陷管理' AS eventType,b.title_ AS title,b.description_ AS description,b.status_ AS status,b.create_time_ AS occurredAt,b.tenant_id_ AS tenantId,r.customer_id_ AS customerId,NULL AS leadId,NULL AS opportunityId,r.id_ AS requirementId,b.id_ AS workItemId,'缺陷管理' AS taskType FROM t_product_bug b JOIN t_product_requirement r ON r.id_=b.requirement_id_ AND r.tenant_id_=b.tenant_id_ AND r.delete_flag_=0 WHERE b.delete_flag_=0 AND r.customer_id_ IS NOT NULL AND r.customer_id_<>'' "
            + "UNION ALL SELECT CONCAT('dev-task:',d.id_) AS id,'研发任务' AS eventType,d.title_ AS title,d.description_ AS description,d.status_ AS status,d.create_time_ AS occurredAt,d.tenant_id_ AS tenantId,r.customer_id_ AS customerId,NULL AS leadId,NULL AS opportunityId,r.id_ AS requirementId,d.id_ AS workItemId,'研发任务' AS taskType FROM t_product_dev_task d JOIN t_product_requirement r ON r.id_=d.requirement_id_ AND r.tenant_id_=d.tenant_id_ AND r.delete_flag_=0 WHERE d.delete_flag_=0 AND r.customer_id_ IS NOT NULL AND r.customer_id_<>''";
    }
    ...
    Map<String,Object> findProject(String id) { List<Map<String,Object>> rows=jdbc.queryForList("SELECT p.id_ AS id,p.engagement_id_ AS engagementId,p.customer_id_ AS customerId,c.name_ AS customerName,p.name_ AS name,p.status_ AS status,p.owner_name_ AS ownerName,p.version_ AS version,p.create_time_ AS createdAt FROM t_project p JOIN t_crm_customer c ON c.id_=p.customer_id_ AND c.tenant_id_=p.tenant_id_ WHERE p.id_=? AND p.tenant_id_=? AND p.delete_flag_=0", id, tenant()); return rows.isEmpty()?null:rows.get(0); }
    int updateProject(String id, Map<String,Object> body) { return jdbc.update("UPDATE t_project SET name_=COALESCE(?,name_),status_=COALESCE(?,status_),owner_name_=COALESCE(?,owner_name_),version_=version_+1,update_time_=NOW() WHERE id_=? AND tenant_id_=? AND delete_flag_=0 AND version_=COALESCE(?,version_)", body.get("name"), body.get("status"), body.get("ownerName"), id, tenant(), body.get("version")); }
