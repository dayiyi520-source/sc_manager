...
public class CrmMapper {
    ...
    private static String journeySql() {
        return "SELECT CONCAT('customer:',c.id_) AS id,'客户建立' AS eventType,c.name_ AS title,CONCAT('客户状态：',COALESCE(c.status_,'')) AS description,c.status_ AS status,c.create_time_ AS occurredAt,c.tenant_id_ AS tenantId,c.id_ AS customerId,NULL AS leadId,NULL AS opportunityId,NULL AS requirementId,NULL AS workItemId,NULL AS taskType FROM t_crm_customer c WHERE c.delete_flag_=0 "
            ...
            + "UNION ALL SELECT CONCAT('work-item:',w.id_,':created') AS id,CASE w.category_ WHEN 'requirement' THEN '需求提交' WHEN 'design' THEN '设计任务' WHEN 'dev' THEN '研发任务' WHEN 'test' THEN '测试任务' ELSE '缺陷管理' END AS eventType,w.title_ AS title,w.description_ AS description,w.status_name_ AS status,w.create_time_ AS occurredAt,w.tenant_id_ AS tenantId,w.customer_id_ AS customerId,NULL AS leadId,NULL AS opportunityId,CASE WHEN w.category_='requirement' THEN w.id_ ELSE w.requirement_id_ END AS requirementId,w.id_ AS workItemId,w.category_ AS taskType FROM t_product_work_item w WHERE w.delete_flag_=0 AND w.customer_id_ IS NOT NULL AND w.customer_id_<>'' "
            + "UNION ALL SELECT CONCAT('work-item-event:',a.id_) AS id,a.event_type_ AS eventType,w.title_ AS title,COALESCE(JSON_UNQUOTE(JSON_EXTRACT(a.content_,'$.reason')),JSON_UNQUOTE(JSON_EXTRACT(a.content_,'$.content')),'') AS description,w.status_name_ AS status,a.create_time_ AS occurredAt,a.tenant_id_ AS tenantId,w.customer_id_ AS customerId,NULL AS leadId,NULL AS opportunityId,CASE WHEN w.category_='requirement' THEN w.id_ ELSE w.requirement_id_ END AS requirementId,w.id_ AS workItemId,w.category_ AS taskType FROM t_product_work_item_activity a JOIN t_product_work_item w ON w.id_=a.subject_id_ AND w.tenant_id_=a.tenant_id_ AND w.delete_flag_=0 WHERE w.customer_id_ IS NOT NULL AND w.customer_id_<>'' "
            + "UNION ALL SELECT CONCAT('presales-task:',p.id_) AS id,'售前任务' AS eventType,p.title_ AS title,p.description_ AS description,p.status_ AS status,p.create_time_ AS occurredAt,p.tenant_id_ AS tenantId,p.customer_id_ AS customerId,NULL AS leadId,NULL AS opportunityId,NULL AS requirementId,p.id_ AS workItemId,'售前任务' AS taskType FROM t_crm_presales_task p WHERE p.delete_flag_=0 AND p.customer_id_ IS NOT NULL AND p.customer_id_<>'' "
            + "UNION ALL SELECT CONCAT('delivery-task:',d.id_) AS id,'交付任务' AS eventType,d.title_ AS title,d.description_ AS description,d.status_ AS status,d.create_time_ AS occurredAt,d.tenant_id_ AS tenantId,d.customer_id_ AS customerId,NULL AS leadId,NULL AS opportunityId,NULL AS requirementId,d.id_ AS workItemId,'交付任务' AS taskType FROM t_project_delivery_task d WHERE d.delete_flag_=0 AND d.customer_id_ IS NOT NULL AND d.customer_id_<>'' "
            + "UNION ALL SELECT CONCAT('ops-task:',d.id_) AS id,'运维任务' AS eventType,d.title_ AS title,d.description_ AS description,d.status_ AS status,d.create_time_ AS occurredAt,d.tenant_id_ AS tenantId,d.customer_id_ AS customerId,NULL AS leadId,NULL AS opportunityId,NULL AS requirementId,d.id_ AS workItemId,'运维任务' AS taskType FROM t_project_ops_task d WHERE d.delete_flag_=0 AND d.customer_id_ IS NOT NULL AND d.customer_id_<>''";
    }
