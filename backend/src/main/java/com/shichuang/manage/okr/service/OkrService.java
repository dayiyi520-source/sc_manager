...
@Service public class OkrService {
 ...
 public Map<String,Object> settings(){AuthorizationService.requireRead("okr");var row=mapper.setting(RequestContext.tenantId());if(row==null)return defaultSettings();var result=payload(row.get("payload"));result.put("version",row.get("version"));return result;}
 @Transactional public Map<String,Object> saveSettings(Map<String,Object> input){AuthorizationService.requireWrite("okr");var clean=defaultSettings();clean.putAll(input);clean.remove("version");String encoded=encode(clean);var current=mapper.setting(RequestContext.tenantId());if(current==null)mapper.insertSetting(RequestContext.tenantId(),UUID.randomUUID().toString(),1,encoded,RequestContext.userId());else if(mapper.updateSetting(RequestContext.tenantId(),((Number)current.get("version")).intValue(),encoded,RequestContext.userId())!=1)throw new ResponseStatusException(HttpStatus.CONFLICT,"配置已变更，请刷新后重试");return settings();}
 private Map<String,Object> defaultSettings(){var result=new LinkedHashMap<String,Object>();result.put("defaultView","list");result.put("timeRules",List.of(Map.of("key","objective","label","添加目标","startDay",1,"endDay",31),Map.of("key","action","label","拆解目标","startDay",1,"endDay",5),Map.of("key","weeklyReview","label","周复盘","startDay",1,"endDay",31),Map.of("key","monthlyReview","label","月复盘","startDay",25,"endDay",5)));result.put("validation",Map.of("actionWeightTotal",100,"maxActions",8,"assigneeMultiple",true,"keyNodeMultiple",false,"resultRequired",true));result.put("dictionaries",Map.of("productNodes",List.of("方案确认","开发完成","测试通过","上线验收"),"deliveryNodes",List.of("项目启动","方案交付","客户验收","正式上线"),"presalesNodes",List.of("需求确认","方案提交","投标截止","客户决策"),"supportTypes",List.of("管理事项","流程优化","人员培养","风险控制","临时专项")));result.put("templates",List.of(Map.of("department","产研部门","type","product","fields",List.of("productLine","title","milestone","assigneeIds","weight","deadline")),Map.of("department","交付部门","type","delivery","fields",List.of("businessObject","title","milestone","assigneeIds","weight","deadline")),Map.of("department","售前部门","type","presales","fields",List.of("businessObject","title","milestone","assigneeIds","weight","deadline")),Map.of("department","其他部门","type","support","fields",List.of("structureType","title","acceptanceStandard","assigneeIds","weight","deadline"))));return result;}
 ...
 public List<Map<String,Object>> actionParents(String period,String requestedViewer){
  ...
  for(var child:actionsById.values()){
   ...
   var item=new LinkedHashMap<String,Object>();item.put("id",parentId);item.put("kind","action");item.put("ownerId",parent.get("ownerId"));item.put("periodKey",period);item.put("status",parent.get("status"));item.put("version",parent.get("version"));item.put("parentObjectiveId",objectiveId);item.put("parentKeyResultId",keyResultId);var sourcePayload=new LinkedHashMap<String,Object>();sourcePayload.put("title",parentPayload.get("title"));sourcePayload.put("parentObjectiveId",objectiveId);sourcePayload.put("parentActionId",parentId);sourcePayload.put("parentKeyResultId",keyResultId);if(parentPayload.get("deadline")!=null)sourcePayload.put("deadline",parentPayload.get("deadline"));item.put("payload",sourcePayload);result.add(item);
  }
  return result;
 }
 ...
 @Transactional public Map<String,Object> createAction(Map<String,Object> body){
  ...
  required(input,"deadline");java.time.LocalDate deadline;try{deadline=java.time.LocalDate.parse(input.get("deadline").toString());}catch(Exception e){throw new IllegalArgumentException("完成时间无效");}String parentDeadline=Objects.toString(parent.get("payload") instanceof Map<?,?> map?map.get("deadline"):"","");if(!parentDeadline.isBlank()&&deadline.isAfter(java.time.LocalDate.parse(parentDeadline)))throw new IllegalArgumentException("完成时间不能晚于上级截止时间");
 ...
 @Transactional public void update(String id,Map<String,Object>b){
  ...
  if("action".equals(kind)){
   ...
   String parentActionId=p.get("parentActionId").toString(), parentObjectiveId=p.get("parentObjectiveId").toString(),parentKeyResultId=Objects.toString(p.get("parentKeyResultId"),parentActionId);var parentRecord=actionParents(s.get("periodKey").toString(),viewer).stream().filter(r->parentActionId.equals(r.get("id"))&&parentObjectiveId.equals(r.get("parentObjectiveId"))&&parentKeyResultId.equals(r.get("parentKeyResultId"))).findFirst().orElse(null);if(parentRecord==null)throw new ResponseStatusException(HttpStatus.FORBIDDEN,"上级动作未指定给当前用户或已变更");String parentDeadline=Objects.toString(payload(parentRecord.get("payload")).get("deadline"),"");if(!parentDeadline.isBlank()&&java.time.LocalDate.parse(p.get("deadline").toString()).isAfter(java.time.LocalDate.parse(parentDeadline)))throw new IllegalArgumentException("完成时间不能晚于上级截止时间");p.put("parentKeyResultId",parentKeyResultId);
