...
export const CRMOpportunitiesView: React.FC = () => {
  ...
  const pagedOpportunities = filteredOpportunities.slice((page - 1) * pageSize, page * pageSize);
  React.useEffect(() => setPage(1), [searchQuery, stageFilter, typeFilter, pageSize]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      <ListToolbar searchPosition="left" searchValue={searchQuery} onSearchChange={setSearchQuery} searchPlaceholder="搜索商机、客户、负责人..." filters={<>
          <select
            value={stageFilter}
            onChange={(e) => setStageFilter(e.target.value)}
            className="app-control min-w-[150px] px-3 text-sm"
          >
          ...
          <button
            ...
            className="app-button-secondary inline-flex h-10 items-center gap-1.5 px-3 text-sm font-medium"
          >
          ...
          <button
            ...
            className="tech-button-primary inline-flex h-10 items-center gap-1.5 rounded-lg px-4 text-sm font-semibold"
          >
      ...
      {/* Table View (表格列: 商机名称、商机类型、客户、金额、负责人、协作人、推进阶段、投标截止日期、赢单率、操作) */}
      {
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-xl shadow-xs overflow-hidden">
          ...
          <Pagination total={filteredOpportunities.length} page={page} pageSize={pageSize} onPageChange={setPage} onPageSizeChange={setPageSize} />
        </div>
      }
      ...
      {activeOpp && (
        ...
        >
          <CRMDetailLayout title="" showHeader={false}>
            ...
            <CRMDetailTabs tabs={[{ key: 'journey', label: '全历程跟进记录' }, { key: 'collab', label: '跨模块联动协作状态' }]} activeKey={oppDetailTab} onChange={setOppDetailTab} />
