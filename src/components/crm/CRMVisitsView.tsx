...
export const CRMVisitsView: React.FC = () => {
  const { customers, addToast } = useAppCrm();
  const remoteDataEnabled = resolveDataMode(readSession()?.token || '', true) === 'remote';
  ...
  const handleSaveVisit = async (e: React.FormEvent) => {
    ...
    if (remoteDataEnabled) {
      try {
        ...
        addToast('error', '拜访计划保存失败', err instanceof Error ? err.message : '请稍后重试');
        return;
      }
    }
  ...
  const handleDrawerCheckIn = async () => {
    ...
    if (remoteDataEnabled) {
      try {
        ...
        addToast('error', '到访打卡失败', err instanceof Error ? err.message : '请稍后重试');
        return;
      }
    }
    ...
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard
          ...
          iconBgColor="bg-blue-50 text-blue-600 border border-blue-200 dark:bg-blue-950/40 dark:text-blue-400 dark:border-blue-800/60"
        />
        <StatCard
          title="已打卡"
          ...
          iconBgColor="bg-emerald-50 text-emerald-600 border border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-400 dark:border-emerald-800/60"
        />
        <StatCard
          title="未打卡"
          value={pendingCount}
          ...
          iconBgColor="bg-amber-50 text-amber-600 border border-amber-200 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-800/60"
        />
      </div>
      ...
      <div className="dark-panel rounded-lg p-5 border border-[var(--border-main)] space-y-4">
        ...
        <div className="border border-[var(--border-main)] rounded-lg overflow-hidden">
          {/* Weekday Header Row */}
          <div className="grid grid-cols-7 border-b border-[var(--border-main)] bg-slate-50 dark:bg-[var(--bg-elevated)]/60 text-center text-xs font-semibold text-[var(--text-muted)] divide-x divide-[var(--border-main)]">
          ...
          {/* Days Grid */}
          <div className="grid grid-cols-7 divide-x divide-y divide-[var(--border-main)] bg-white dark:bg-[var(--bg-main)]">
            {calendarGrid.map((cell, idx) => {
              if (!cell) {
                return (
                  <div
                    key={`empty-${idx}`}
                    className="min-h-[110px] bg-slate-50/70 dark:bg-[var(--bg-main)]/30 opacity-60 p-2"
                  />
                );
              }
              ...
              return (
                <div
                  key={dateStr}
                  className="min-h-[110px] p-2 relative group hover:bg-slate-50/80 dark:hover:bg-[var(--bg-elevated)]/40 transition-colors flex flex-col justify-between"
                >
                  {/* Top Day Header inside Cell */}
                  <div className="flex items-center justify-between mb-1.5">
                    {isHighlightDay ? (
                      <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-blue-600 text-white font-bold text-xs shadow-xs">
                        {dayNumber}
                      </span>
                    ) : (
                    ...
                    {dayPlans.map((plan) => {
                      ...
                      return (
                        <div
                          ...
                          className={`p-1.5 rounded-md border text-xs font-medium cursor-pointer transition-all flex items-center justify-between gap-1 shadow-2xs ${
                            isChecked
                              ? 'bg-emerald-50 text-emerald-800 border-emerald-200 hover:bg-emerald-100/90 hover:border-emerald-300 dark:bg-emerald-950/50 dark:text-emerald-300 dark:border-emerald-500/40 dark:hover:border-emerald-400 dark:hover:bg-emerald-900/60'
                              : 'bg-amber-50 text-amber-900 border-amber-200 hover:bg-amber-100/90 hover:border-amber-300 dark:bg-amber-950/50 dark:text-amber-300 dark:border-amber-500/40 dark:hover:border-amber-400 dark:hover:bg-amber-900/60'
                          }`}
                        ...
                        >
                          <div className="truncate flex items-center gap-1">
                            ...
                            {visitorCount > 1 && (
                              <span className="text-[10px] opacity-80 shrink-0 px-1 rounded bg-black/5 text-slate-700 dark:bg-black/30 dark:text-amber-200 font-mono">
                                {visitorCount}人
                              </span>
                            )}
                          </div>
        ...
        {selectedPlan && (
          <div className="space-y-5 text-xs">
            ...
            {selectedPlan.status === '已打卡' ? (
              <div className="p-4 rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300 space-y-2">
                <div className="flex items-center gap-2 font-bold text-sm text-emerald-700 dark:text-emerald-400">
                  <CheckCircle2 className="w-4 h-4 shrink-0" />
                  已完成现场定位打卡归档
                </div>
                <div className="text-[11px] text-emerald-800/90 dark:text-emerald-200/80 space-y-1">
                  ...
                  {selectedPlan.checkInNotes && (
                    <p className="pt-1.5 border-t border-emerald-200 dark:border-emerald-500/20 text-emerald-900 dark:text-emerald-100">
