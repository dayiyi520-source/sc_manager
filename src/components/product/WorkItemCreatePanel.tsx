...
export const WorkItemCreatePanel: React.FC<{
  ...
  if (presentation === 'drawer') {
    return <Drawer open onClose={onClose} title={title} size="min(1480px, 92vw)" destroyOnHidden footer={footer ? <div className="flex items-center gap-3">{secondaryAction}<span className="flex-1" />{footer}</div> : undefined}>
      <div className={properties ? 'grid min-h-full lg:grid-cols-[minmax(0,1fr)_320px]' : 'min-h-full'}><main className="min-w-0 px-2 py-2">{editor || children}</main>{properties && <aside className="border-l border-[var(--border-main)] bg-[var(--bg-card)] px-6 py-6" aria-label="字段设置">{properties}</aside>}</div>
    </Drawer>;
  }
  return (
