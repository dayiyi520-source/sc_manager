...
export const TestCaseLibraryView: React.FC<TestCaseLibraryViewProps> = ({ productLineFilter = 'all' }) => {
  ...
  const batchUpdate = useMutation({
    mutationFn: () => productRepository.batchUpdateTestCases(lineId, { caseIds: selectedCaseIds.map(String), operation: batchAction!.toUpperCase() as 'MOVE' | 'OWNER' | 'PRIORITY', value: batchValue }),
  ...
  const handleBatchAction = (value: string) => {
    if (value === 'delete') {
      Modal.confirm({ title: '确认删除用例？', content: `将删除选中的 ${selectedCaseIds.length} 条用例，已被测试任务引用的用例不会被删除。`, okText: '确认删除', cancelText: '取消', okButtonProps: { danger: true }, onOk: async () => { try { await productRepository.batchUpdateTestCases(lineId, { caseIds: selectedCaseIds.map(String), operation: 'DELETE' }); setSelectedCaseIds([]); await Promise.all([cases.refetch(), directories.refetch()]); } catch (reason) { setActionError(reason instanceof Error ? reason.message : '批量删除失败'); } } });
      return;
    }
  ...
  return <div className="test-case-library">
    ...
    <div className="test-case-library-grid">
      ...
      <main className="test-case-data-plane">
        {selectedCaseIds.length > 0 && <div className="test-case-batch-bar"><span>已选中 {selectedCaseIds.length} 条用例</span><Select value={undefined} placeholder={`批量操作${selectedCaseIds.length}条`} onChange={handleBatchAction} options={[{ label: '移动用例', value: 'move' }, { label: '删除用例', value: 'delete' }, { label: '修改负责人', value: 'owner' }, { label: '修改优先级', value: 'priority' }, { label: '修改类型（类型固定）', value: 'type', disabled: true }]} /><Button onClick={() => setSelectedCaseIds([])}>取消选择</Button></div>}
    ...
    <TestCaseEditorDrawer open={editorOpen} productLineId={lineId} directories={directories.data || []} initialCase={editingCase} onClose={() => setEditorOpen(false)} onSaved={(_saved, continueCreating) => { if (!continueCreating) setEditorOpen(false); void cases.refetch(); void directories.refetch(); }} />
  </div>;
};
