...
export const VersionTestReportPanel: React.FC<Props> = ({ productLineId, versionId, versionName }) => {
  ...
  const reports = useQuery({ queryKey: ['version-test-reports', productLineId, versionId], queryFn: () => productRepository.versionTestReports(productLineId, versionId), enabled: hasContext, retry: false });
  const plans = useQuery({ queryKey: ['version-test-report-plans', productLineId, versionId], queryFn: () => productRepository.versionTestReportPlans(productLineId, versionId), enabled: hasContext, retry: false });
  const detail = useQuery({ queryKey: ['version-test-report', productLineId, versionId, selectedId], queryFn: () => productRepository.versionTestReport(productLineId, versionId, selectedId), enabled: hasContext && !!selectedId, retry: false });
