      ...
      return <section className="test-plan-item" key={plan.id}>
        ...
        {expanded && <div className="test-plan-expanded">
          ...
          <Table<TestPlanCase> size="small" rowKey="testCaseId" dataSource={plan.cases} pagination={false} locale={{ emptyText: '尚未引用测试用例' }} columns={[
            { title: '编号', dataIndex: 'code', width: 116, render: (value) => <span className="test-case-code">{value}</span> }, { title: '标题', dataIndex: 'title', ellipsis: true }, { title: '优先级', dataIndex: 'priority', width: 88, render: (value) => <Tag>{value}</Tag> }, { title: '执行人', dataIndex: 'ownerName', width: 140, render: (value) => <PersonIdentity name={value} emptyLabel="未设置" variant="list" /> },
