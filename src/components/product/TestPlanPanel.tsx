...
export const TestPlanPanel: React.FC<TestPlanPanelProps> = ({ task, workItemId, productLineId, sourceRequirementId, employeeNames, onExecutionCreated }) => {
  ...
  return <div className="test-plan-panel">
    ...
    <div className="test-task-section-heading"><div><h3>测试计划</h3><span>共 {plans.data?.length || 0} 条，执行结果由测试人员逐条填写</span></div><Button type="primary" icon={<PlusOutlined />} onClick={openCreatePlan}>创建测试计划</Button></div>
    {!plans.data?.length ? <div className="test-plan-empty"><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="尚未创建测试计划"><Button type="primary" icon={<PlusOutlined />} onClick={openCreatePlan}>创建测试计划</Button></Empty></div> : plans.data.map((plan) => {
