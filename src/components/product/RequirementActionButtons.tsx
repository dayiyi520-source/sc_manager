...
export const RequirementActionButtons: React.FC<{
  ...
  return <div className="flex gap-2">
    ...
    {status !== '已搁置' && <button type="button" onClick={onHold} className="h-9 px-3 rounded-lg border border-[var(--border-main)] text-xs text-[var(--text-body)]"><Ban className="mr-1 inline w-3.5 h-3.5" />事项搁置</button>}
    <button type="button" onClick={onReject} className="h-9 px-3 rounded-lg border border-[var(--danger)] text-xs text-[var(--danger)]">事项驳回</button>
  </div>;
};
