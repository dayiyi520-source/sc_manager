...
describe('cycle groups',()=>{
 ...
 it('groups the current month, historical months, and next month by calendar state',()=>{
  ...
  expect(groups.map(group=>group.label)).toEqual(['进行中','未开始','已结束']);
  expect(groups[0].children.map(c=>c.value)).toEqual(['2026-09']);
  expect(groups[1].children.map(c=>c.value)).toEqual(['2026-10']);
  expect(groups[2].children.map(c=>c.value)).toEqual(expect.arrayContaining(['2026-08','2026-07','2026-06','2026-05']));
 });
});
