...
import { Button, DatePicker, Flex, Form, Input, Modal, Select, Space, Table, Tag, Typography } from 'antd';
...
export function ReviewEditor({record,objectives,work,busy,onSave,onClose}:{record?:OkrRecord;objectives:OkrRecord[];work:OkrWork[];busy:boolean;onSave:(p:OkrPayload)=>Promise<void>;onClose:()=>void}) {
 ...
 return <Modal open width="90%" title={record?'修改复盘':'周期复盘'} onCancel={onClose} footer={null} destroyOnHidden>
  <Form form={form} disabled={busy} layout="vertical" initialValues={{title:record?.payload.title,summary:record?.payload.summary,period:record?[dayjs(record.payload.startDate),dayjs(record.payload.endDate)]:[dayjs().startOf('month'),dayjs().endOf('month')]}} onFinish={v=>onSave({title:v.title,startDate:v.period[0].format('YYYY-MM-DD'),endDate:v.period[1].format('YYYY-MM-DD'),summary:v.summary||'',items})}>
   ...
   <Flex wrap justify="space-between" gap="middle" className="mb-4"><Typography.Text>工作项进展 · {items.length} 项 · 计划外 {items.filter(i=>!i.objectiveId).length} 项</Typography.Text><Button onClick={()=>setItems(candidates.map(w=>reviewWork(w,items.find(i=>i.workId===w.id))))}>归集本期工作项</Button></Flex>
   <Select className="w-full mb-4" mode="multiple" showSearch optionFilterProp="label" placeholder="关联任务或事项" value={items.map(i=>i.workId)} options={candidates.map(w=>({value:w.id,label:`${w.title} · ${w.status}`}))} onChange={ids=>setItems(ids.map(id=>reviewWork(work.find(w=>w.id===id)!,items.find(i=>i.workId===id))))}/>
   <Table rowKey="workId" dataSource={items} pagination={{pageSize:5}} scroll={{x:900}} columns={[
    {title:'任务 / 事项',dataIndex:'title',width:220,render:(title,row)=><><div>{title}</div><Tag>{row.status}</Tag></>},
   ...
   <Flex justify="end" gap="small"><Button onClick={onClose} disabled={busy}>取消</Button><Button type="primary" htmlType="submit" loading={busy}>保存草稿</Button></Flex>
  </Form>
 </Modal>;
}
