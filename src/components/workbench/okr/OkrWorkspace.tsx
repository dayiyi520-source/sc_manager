...
import { Alert, Button, DatePicker, Descriptions, Drawer, Empty, Form, Input, InputNumber, Modal, Progress, Select, Space, Spin, Table, Tabs, Tag, Timeline } from 'antd';
...
export function OkrWorkspace(){
 const {currentUser,addToast}=useApp();const client=useQueryClient();const [cycle,setCycle]=useState(dayjs().format('YYYY-MM'));
 ...
 const run=async(action:()=>Promise<unknown>)=>{setBusy(true);try{await action();await refresh();addToast('success','保存成功');return true;}catch(e){addToast('error',e instanceof Error?e.message:'保存失败');return false;}finally{setBusy(false);}};
 ...
 return <div className="okr-workspace space-y-4 text-[var(--text-primary)]">
  <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold">目标与绩效</h2><Space wrap><DatePicker picker="month" value={dayjs(cycle)} allowClear={false} onChange={v=>{if(v)setCycle(v.format('YYYY-MM'));}}/><Button icon={<SyncIcon/>} title="刷新" aria-label="刷新" onClick={()=>refresh()}/>{['objective','review'].includes(tab)&&<Button icon={<PlusIcon/>} type="primary" onClick={()=>setEditing('new')}>{tab==='review'?'新建复盘':'新增目标'}</Button>}</Space></div>
  {!me?.supervisorId&&!me?.rootFlag&&<Alert type="warning" title="尚未配置直属上级" description="请由管理员在组织关系中配置承接关系。"/>}
  <Tabs activeKey={tab} onChange={v=>{setTab(v);setScope('my');setEditing(undefined);}} items={[{key:'objective',label:'目标 OKR'},{key:'work',label:'任务与工单'},{key:'review',label:'周期复盘'},...(currentUser.role==='admin'?[{key:'organization',label:'组织关系'}]:[])]}/>
  {tab==='work'?work.isError?<Alert type="error" title="工作项加载失败" description={work.error.message} action={<Button onClick={()=>work.refetch()}>重试</Button>}/>:<WorkLinks work={work.data||[]} objectives={objectives} run={run} busy={busy}/>:tab==='organization'?<Table rowKey="id" dataSource={people.data} columns={[{title:'成员',dataIndex:'name'},{title:'部门',dataIndex:'department'},{title:'直属上级',render:(_,p:OkrPerson)=>p.rootFlag?'组织根目标负责人':people.data?.find(x=>x.id===p.supervisorId)?.name||'未配置'},{title:'操作',render:(_,p:OkrPerson)=><Button onClick={()=>{setOrgPerson(p);setSupervisor(p.rootFlag?'root':p.supervisorId||undefined);}}>配置</Button>}]}/>:<>
   <Space><Select value={scope} onChange={setScope} options={[{value:'my',label:tab==='review'?'我的复盘':'我的目标'},...(tab==='objective'?[{value:'supervisor',label:'上级目标'}]:[]),{value:'received',label:tab==='review'?'待我评价 / 下属复盘':'下属目标'}]}/><span>{visible.length} 项</span></Space>
  ...
  <Drawer open={!!detail} width="80%" title={detail?.payload.title} onClose={()=>setDetailId(undefined)} extra={detail&&actions(detail)}>
   {detail&&<div className="space-y-4"><Descriptions items={[{key:'state',label:'状态',children:statusNames[detail.status]},{key:'period',label:'周期',children:detail.periodKey},{key:'parent',label:'承接目标',children:all.find(r=>r.id===detail.payload.parentObjectiveId)?.payload.title||'组织根目标 / 无关联'}]}/>
    {detail.kind==='objective'?<><Table rowKey="id" pagination={false} dataSource={detail.payload.keyResults} columns={[{title:'关键结果',dataIndex:'title'},{title:'权重',dataIndex:'weight',render:v=>`${v}%`},{title:'进度',render:(_,kr)=><InputNumber key={`${kr.id}-${detail.version}`} aria-label={`${kr.title}进度`} min={0} max={100} precision={0} defaultValue={kr.progress} disabled={!isOwner(detail)||detail.status!=='active'||busy} onBlur={e=>{const v=Number(e.target.value);if(e.target.value!==''&&Number.isInteger(v)&&v>=0&&v<=100&&v!==kr.progress)run(()=>okrRepository.update(detail,'progress',{keyResults:detail.payload.keyResults?.map(k=>k.id===kr.id?{...k,progress:v}:k)}));}}/>}]}/>
     <h3>下级承接</h3>{all.filter(r=>r.payload.parentObjectiveId===detail.id).map(r=><div key={r.id}>{r.payload.title}</div>)}</>:<>
     <ReviewDetails record={detail} objectives={all.filter(o=>o.kind==='objective'&&o.ownerId===detail.ownerId)}/>
    </>}
    <h3>动态记录</h3><Timeline items={events.data?.map((e,i)=>({key:i,content:`${e.createdAt} · ${e.operator} · ${e.action}`}))}/>
   </div>}
  </Drawer>
