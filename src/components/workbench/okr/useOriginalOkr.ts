...
export function useOriginalOkr() {
  ...
  const save = async (kind: 'objective' | 'review', period:string, payload:OkrPayload) => {
    setBusy(true);
    try {
      await okrRepository.create(kind,period,payload,true);
      await refresh(); addToast('success',kind==='objective'?'目标已提交':'复盘已提交'); return true;
