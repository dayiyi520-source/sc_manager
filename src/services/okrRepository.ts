...
export interface OkrRecord { id: string; kind: 'objective' | 'review'; ownerId: string; periodKey: string; status: string; version: number; payload: OkrPayload }
...
export const okrRepository = {
  ...
  create: (kind: string, periodKey: string, payload: OkrPayload) => apiRequest<{id:string}>(`${base}/records`, {method:'POST',body:JSON.stringify({kind,periodKey,payload})}),
